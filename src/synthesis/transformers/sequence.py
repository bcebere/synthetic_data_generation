...
class GeneralizeDateSequence(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        if self.date_sequence is None:
            ...
            print(
                "no data_sequence specified in init - assume columns in X "
    ...
    def inverse_transform(self, Xt):
        ...
        assert Xt.columns.isin(
            self.date_sequence
        ).all(), "input dataframe contains columns not seen in fit"

        if not pd.core.dtypes.common.is_period_dtype(Xt[self.reference_date_col]):
            Xt[self.reference_date_col] = pd.to_datetime(
                Xt[self.reference_date_col].astype(str)
            ).dt.to_period("m")
        ...
        reference_date_ymd = pd.to_datetime(
            {
                "year": reference_period_ym.dt.year,
        ...
        for c in self.date_sequence[1:]:
            ...
            Xinv[c] = reference_date_ymd + pd.to_timedelta(
                Xinv[c].astype(float), unit="d"
            )
