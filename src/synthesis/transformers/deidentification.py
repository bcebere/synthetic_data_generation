...
def replace_unique_values(
    df: pd.DataFrame, column_name: str, replace_string: str = "entity_"
) -> pd.DataFrame:
    """Replaces unique values with numbered string. Retains nans."""
    df[column_name] = df[column_name].astype(str)
    replace_dict = {
        v: (replace_string + str(i) if v != "nan" else np.nan)
        for i, v in enumerate(df[column_name].unique())
    }
...
@pf.register_dataframe_method
def group_rare_values(
    df: pd.DataFrame, column_names=None, threshold=0.05, rare_group_label="other"
):
    ...
    for c in column_names:
        df.loc[:, c] = df[c].mask(
            df[c].map(df[c].value_counts(normalize=True)) < threshold, rare_group_label
        )
    return df
