...
from synthesis.synthesizers.utils import (
    dp_marginal_distribution,
...
def bin_numeric_column(
    ...
    df.loc[~mask_missing, column_name] = pd.arrays.IntervalArray.from_arrays(
        lower_bound, upper_bound, closed="left"
    )
...
@pf.register_dataframe_method
def sample_from_binned_column(
    df: pd.DataFrame, column_name: str, numeric_type="int", mean=None, std=None
):
    ...
    try:
        ...
        raise ValueError(
            "Numeric column needs to be converted to bins first. "
    ...
    # sample from uniform distribution
        ...
        if numeric_type == "int":
            df.loc[~mask_missing, column_name] = np.random.randint(
                lower_bound, upper_bound
            )
        elif numeric_type == "float":
            df.loc[~mask_missing, column_name] = np.random.uniform(
                lower_bound, upper_bound
            )
    return df
...
def replace_rare_values(df, column, min_support=0.01, new_name="other"):
    ...
    print(
        f"Categories in column '{column}' that occur less than {n_records_support}x "
    ...
    convert_low_freq_rows = df[column].mask(
        map_value_probs_to_rows < min_support, new_name
    )
...
class GeneralizeContinuous(KBinsDiscretizer):
    ...
    def transform(self, X):
        ...
        for jj in range(Xt.shape[1]):
            ...
            Xt[~missing_idx, jj] = np.digitize(
                Xt[~missing_idx, jj] + eps, bin_edges[jj][1:]
            )
    ...
    def inverse_transform(self, Xt):
        ...
        assert set(Xt.columns) == set(
            self._header
        ), "input contains different columns than seen in fit"

        Xinv = check_array(
            Xt, copy=True, dtype=FLOAT_DTYPES, force_all_finite="allow-nan"
        )
        # Xinv = Xt.copy()
        n_features = self.n_bins_.shape[0]
...
class GeneralizeCategorical(GeneralizeContinuous):
    ...
    def fit(self, X, y=None):
        ...
        for jj, c in enumerate(X.columns):
            self.marginals_.append(
                dp_marginal_distribution(X_enc.loc[:, c], local_epsilon).values
            )
    ...
    def inverse_transform(self, Xt):
        ...
        for jj, c in enumerate(Xt.columns):
            ...
            np.clip(
                marginal_group_alloc, 0, self.n_bins_[jj] - 1, out=marginal_group_alloc
            )
            ...
            for i in range(n_records):
                ...
                marginal_candidate_idx = np.where(X_enc[i, jj] == marginal_group_alloc)[
                    0
                ]
                marginal_candidate_probs = marginals[marginal_candidate_idx]
                marginal_candidate_probs_normalized = _normalize_distribution(
                ...
                X_enc[i, jj] = np.random.choice(
                    marginal_candidate_idx, p=marginal_candidate_probs_normalized
                )
...
class GroupRareCategories(BaseReversibleTransformer):
    ...
    def transform(self, X, y=None):
        ...
        return X.mask(
            X.map(X.value_counts(normalize=True)) < self.threshold, self.name_group
        )
