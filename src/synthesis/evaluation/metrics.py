...
class MarginalComparison(BaseMetric):
    ...
    def fit(self, data_original, data_synthetic):
        data_original, data_synthetic = self._check_input_data(
            data_original, data_synthetic
        )
        ...
        for c in data_original.columns:
            ...
            self.stats_original_[c], self.stats_synthetic_[c] = (
                data_original[c]
                .value_counts(dropna=False, normalize=self.normalize)
                .align(
                    data_synthetic[c].value_counts(
        ...
        for idx, col in enumerate(column_names):
            ...
            if len(column_value_counts_original.values) <= 25:
                ax[idx].set_xticklabels(
                    column_value_counts_original.keys(), rotation=25
                )
            else:
                ax[idx].set_xticklabels("")
...
class AssociationsComparison(BaseMetric):
    def __init__(
        ...
        super().__init__(
            labels=labels, exclude_columns=exclude_columns, astype_cat=False
        )
    ...
    def fit(self, data_original, data_synthetic):
        ...
        self.stats_original_ = associations(
            data_original,
        ...
        self.stats_synthetic_ = associations(
            data_synthetic,
            nom_nom_assoc=self.nom_nom_assoc,
            nominal_columns=self.nominal_columns,
            nan_replace_value="nan",
    ...
    def score(self):
        pairwise_correlation_distance = np.linalg.norm(
            self.stats_original_ - self.stats_synthetic_, "fro"
        )
...
class JointDistributionComparison(BaseMetric):
    def __init__(
        self, labels=None, exclude_columns=None, normalize=True, n_variables=2
    ):
        super().__init__(labels=labels, exclude_columns=exclude_columns)
        self.normalize = normalize
    ...
    def fit(self, data_original, data_synthetic):
        ...
        for vars in variable_combinations:
            ...
            jt_original = utils.compute_distribution(
                data_original.loc[:, vars]
            ).as_series()
            jt_synthetic = utils.compute_distribution(
                data_synthetic.loc[:, vars]
            ).as_series()

            # align both original and synthetic in case values are missing in either dataset
            jt_original, jt_synthetic = jt_original.align(
                jt_synthetic, join="outer", fill_value=0, axis=0
            )
