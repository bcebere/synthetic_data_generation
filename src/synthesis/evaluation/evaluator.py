        ...
        if self.metrics is not None:
            for name, metric in self.metrics.items():
                if not isinstance(metric, BaseMetric):
                    raise ValueError(
                        "Input metric {} should subclass synthesis.evaluation._base.BaseMetric".format(
                            metric
                        )
                    )
        else:
...
class OriginalDataEvaluator:
    ...
    def fit(self, data):
        self.stats_ = {}
        self.stats_["columns_high_cardinality"] = self.get_high_cardinality_columns(
            data, self.cardinality_threshold
        )
        self.stats_["rare_column_categories"] = self.get_rare_column_categories(
            data, self.rare_category_threshold
        )
        return self
    ...
    def plot(self, data, normalize=True):
        ...
        for idx, col in enumerate(column_names):
            ...
            ax[idx].bar(
                x=bar_position,
                height=column_value_counts.values,
    ...
    def get_rare_column_categories(data, threshold):
        ...
        for c in data.columns:
            rare_categories[c] = [
                k
                for k, v in data[c].value_counts(normalize=True).items()
                if v < threshold
            ]
