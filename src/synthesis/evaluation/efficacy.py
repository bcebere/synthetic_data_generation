...
class TrainBothTestOriginalHoldout(BasePredictiveMetric):
    ...
    def _fit(self, X, y):
        ...
        numeric_features = [
            feat for feat in X.columns if not feat in categorical_features
        ]
        ...
        preprocessor_cat = Pipeline(
            steps=[
                ...
                (
                    ...
                    EnsureConsistentType(dtype=str),
        ...
        # Pipeline
        pipe = Pipeline([("preprocessor", preprocessor), ("classifier", clf_rf)])
    ...
    def score(self, data_original_test):
        ...
        scores = {
            "roc_auc_original": roc_auc_score(
                y_test, self.stats_original_.predict_proba(X_test)[:, 1]
            ),
            "roc_auc_synthetic": roc_auc_score(
                y_test, self.stats_synthetic_.predict_proba(X_test)[:, 1]
            ),
        }
        return scores
    ...
    def plot(self, data_original_test):
        ...
        ax.plot([0, 1], [0, 1], linestyle="--", lw=1, color="black", alpha=0.8)
        plt.title("ROC Curve")
        plt.show()
